...
All templates will be places later there.
...
Then add a new DI container definition as follows:
...
return [
    ...
    TemplateRenderer::class => function (ContainerInterface $container) {
        $renderer = new TemplateRenderer($container->get(Engine::class));
        $settings = $container->get('settings');
        $renderer->setTemplatePath((string)$settings['template']);
