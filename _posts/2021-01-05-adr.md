...
Putting all the response-building in a single class with multiple methods, 
...
The request passes through the [middleware](https://www.slimframework.com/docs/v4/concepts/middleware.html)
stack (in and out):
...
You can more easily **keep up** with the change rate of the framework-specific code or replace the framework altogether.
...
What I also do to comply with SOLID is to **separate the behavior from the data**.
I put the logic into services and the plain data into tiny and domain specific objects (e.g. DTO's).
This allows me to build and maintain non-trivial applications for many years. 

You may think that this is not classic OOP, and you are right, 
because this style is therefore also called Service-Oriented Architecture (SOA).
...
In SOA, the separation between data and behavior is often intentional.
...
The OOP example from above shows that the OOP approach violates SOLID's SRP because the 
`Account` class acts as a kind of entity AND is responsible for holding its own context 
data AND does validation AND various business operations on itself.

The SOA style instead respects all the [SOLID](https://scotch.io/bar-talk/s-o-l-i-d-the-first-five-principles-of-object-oriented-design) 
principles and is even more [TDD](https://hackernoon.com/introduction-to-test-driven-development-tdd-61a13bc92d92) friendly as well.
...
* [Wikipedia: Action Domain Responder](https://en.wikipedia.org/wiki/Action%E2%80%93domain%E2%80%93responder)
