...
[Vue.js](https://vuejs.org/) is an open-source model–view–viewmodel JavaScript framework for building 
...
Vue and [Twig](https://twig.symfony.com/) use the same template syntax `{{ }}`. 
Twig will parse the templates before Vue can render it in the browser.
To fix it you could change the Vue delimiters to `[[ ]]` with: `Vue.options.delimiters = ['[[', ']]'];`
or you can drop Twig completely in favor of a native PHP template engine like [Plates](https://platesphp.com/).
